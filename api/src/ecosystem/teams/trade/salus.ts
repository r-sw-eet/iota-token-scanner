...
- **Visual attestation:** Salus publishes images of their on-chain NFTs (DWR receipts) on their X/Twitter account. Anyone can cross-check a shown NFT's address against the scanner's \`0x4876…c34225\` deployer.
...
Scan of deployer \`0x4876…c34225\` reveals 60 distinct package addresses, all with a single module named \`nft\`. Unusual pattern: rather than one package upgraded 60 times (which would preserve the original address with version bumps), Salus publishes a *new* package per batch of DWRs. The registry's \`packageAddresses\` hardcode of one specific address (\`0xf5e4f559…a90f\`) catches that one instance; everything else is rescued via the fingerprint rule.
...
- [x] No other deployer on IOTA mainnet mints NFT objects with \`issuer=0x4876…c34225\` and \`tag="salus"\`.
