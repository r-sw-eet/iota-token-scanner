...
Side-finding: CyberPerp's trading deployer (\`0x14ef…c3e0\`) publishes 4 packages each containing a module literally named \`pyth\`. Those are CyberPerp's **client-side Pyth price-feed integration** (trading engines embed a consumer of Pyth's oracle), not Pyth's oracle contract itself. The Pyth match rule \`{all: [batch_price_attestation]}\` correctly disambiguates.

Triangulation:
- [x] Pyth's official docs publish both package addresses verbatim.
- [x] On-chain scan confirms both packages exist at deployer \`0x610a…6d27\`.
