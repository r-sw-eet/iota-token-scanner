...
Resolution: queried \`::stake_config::GlobalConfig\` object \`0xad0c222b5bfe…\` and read its admin fields:
...
The vanity-prefix address \`0x5555…ae7c\` holds all three admin roles. Scanning packages deployed by that address returned 4 additional packages with telling module names:

- \`0xb63c0471…\` — \`tln_token\` module (coin type **TLN_TOKEN** — TokenLabs' native utility token).
- \`0xaa560ead…\` — \`simple_payment\` module.
- \`0xe4abf8b6…\` — \`{cert, math, native_pool, ownership, validator_set}\` — a **liquid staking** contract (vIOTA).
- \`0x6ab984df…\` — same 5 modules (vIOTA upgrade).

Web search for "TLN token" IOTA mainnet surfaces \`tokenlabs.network\` immediately — "an IOTA Rebased Validator & DeFi Staking Platform." GeckoTerminal confirms a TLN/stIOTA pool live on Pools Finance at \`0x168669080aafe…7320f\` (cross-protocol DeFi composability with Swirl).

On-chain product breakdown:

**Engine deployer \`0x9bd8…9841\`** — ships the staking framework (\`{stake, stake_config, stake_entries}\` × 3 upgrade versions). Liquidswap-style multi-pool architecture:
\`\`\`
...
**Admin/Operator \`0x5555…ae7c\`** — ships the operational product surface:
- **TLN Token** (\`0xb63c0471…\`) — the coin type.
- **Liquid Staking v1 / v2** (\`0xe4abf8b6…\`, \`0x6ab984df…\`) — TokenLabs' vIOTA product.
- **Simple Payment** (\`0xaa560ead…\`).
...
Triangulation:
- [x] On-chain admin probe resolves to vanity address \`0x5555…ae7c\`.
